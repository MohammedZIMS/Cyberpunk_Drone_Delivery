export class AudioManager {
  constructor() {
    ...
    this._ready      = false;
    this._suspended  = false; // tracks our intentional pause
  }
  ...
  /**
   * Suspend the Web Audio clock — all oscillators and buffer sources freeze
   * exactly where they are and resume from the same point.
   */
  pauseAll() {
    if (!this._ready || this._suspended) return;
    this._ctx.suspend();
    this._suspended = true;
  }

  /**
   * Resume the Web Audio clock from the suspended point.
   * Safe to call even if not suspended (no-op).
   */
  resumeAll() {
    if (!this._ready || !this._suspended) return;
    this._ctx.resume();
  ...
  // ── Per-frame mix (only runs when NOT paused) ─────────────────────────────
  ...
  update(droneSpeed, windStrength, rainAmount) {
    ...
    // Rain and wind scale with weather intensity
  ...
  /** Play an ascending or descending chord arpeggio. */
